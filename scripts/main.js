...
var ProcessWireAdminTheme = {
	...
	ready: function() {
		...
		$(document).on('wiretabclick opened', function(e) {
			...
			$(window).trigger('resize'); // force Customer3000 to update grid
		...
		$('#_ProcessPageEditView').on('click', function(e) {
			// Customer3000 tab blocks this link, so this allows it through
			e.stopPropagation();
		});
	...
	setupSideNav: function() {
		$('.pw-sidebar-nav').on('click', 'a', function(event) {
			...
			if(!$a.hasClass('pw-has-ajax-items')) {
				// as of Customer3000 beta 34, clicking nav items closes the offcanvas unless the following line is here
				event.stopPropagation();
				return;
			}
			...
			$.getJSON(url, function(data) {
				...
				$.each(data.list, function(i) {
					if(this.label.indexOf('<span') > -1) {
						// Customer3000 beta 34 does not like span elements in the label for some reason
					...
					if(this.navJSON != "undefined" && this.navJSON) {
						...
						Customer3000.nav($ul2, { multiple: true });
					}
	...
	/**
	 * Initialize Inputfield forms and Inputfields for Customer3000
	 * 
	 */
	setupInputfields: function() {
		...
		function initFormMarkup($target) {
			...
			$('.MarkupPagerNav:not(.uk-pagination)').each(function() {
			 ...
			 // apply to inputs that don’t already have Customer3000 classes 
	...
	/**
	 * Initialize tooltips, converting jQuery UI tooltips to Customer3000 tooltips before they get init'd by jQuery
	 * 
	 */
	setupTooltips: function() {
		$('.tooltip, .pw-tooltip').each(function() {
			$(this).removeClass('tooltip pw-tooltip');
			Customer3000.tooltip($(this));
		});
	},
	...
	logoClickEvent: function() {
		if($('body').hasClass('pw-layout-sidenav-init')) {
			if($('#pw-admin-side').length) {
				...
				// show offcanvas nav
				Customer3000.toggle('#offcanvas-nav').toggle();
			}
